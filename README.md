...
Remarque : Si `VsCode` est déjà installer dans votre pc vous pouvez passer directement à l'étape `IV)`.

1. Rendez-vous sur : `https://code.visualstudio.com/`

2. Cliquez sur : `Download` (en haut à droite en bleue)

3. Suivez les étapes de l'installation par défaut

# IV) Pré-requis (NodeJs) :

Remarque : Si `NodeJs` est déjà installer dans votre pc vous pouvez passer directement à l'étape `V)`.
...
3. Se rendre dans le dossier `backend` à l'aide du `terminal`en tapant `cd backend`(sans les backtick idem pour la suite des opérations depuis le `terminal`) puis valider à l'aide de la touche `entrer` du clavier

4. Tapez (toujours dans le terminal) : `npm install` puis valider à l'aide de la touche `entrer` du clavier

5. Tapez (toujours dans le termninal et dossier backend) : `npm install --save-dev sequelize-cli` puis valider à l'aide de la touche `entrer` du clavier
...
10. Retourner dans le terminal (encore dans le dossier backend) et tapez : `npx sequelize-cli model:generate --name User --attributes username:string,email:string,password:string,isAdmin:boolean,imageProfile:string` puis valider à l'aide de la touche `entrer` du clavier

Remarque : un ficher model et migration sont créer dans les dossier `migrations`et `.models`

11. toujours dans le terminal (le dossier backend) et tapez : `npx sequelize-cli model:generate --name Post --attributes userId:integer,content:string,imagePost:string,likes:string` puis valider à l'aide de la touche `entrer` du clavier

12. toujours dans le terminal (le dossier backend) et tapez : `npx sequelize-cli model:generate --name Comment --attributes postId:integer,userId:integer,content:string` puis valider à l'aide de la touche `entrer` du clavier

13. toujours dans le terminal (le dossier backend) et tapez : `npx sequelize-cli model:generate --name Like --attributes postId:integer,userId:integer` puis valider à l'aide de la touche `entrer` du clavier
...
16. Toujours dans le terminal (et toujours dans le dossier backend), tapez : `npx sequelize-cli db:migrate` puis valider à l'aide de la touche `entrer` du clavier
